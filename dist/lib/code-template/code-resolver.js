...
const findForeignKey = (tableItem, keyColumnList, inputCol = '') => {
  ...
  async  ${(0, lodash_1.camelCase)(p.tableName)}${(0, helper_1.pascalCase)(p.columnName)}(
    ...
    const service = await ctx.reqCtx.requestContext.getAsync<${(0, helper_1.pascalCase)(p.tableName)}Service>('${(0, lodash_1.camelCase)(p.tableName)}Service');
  ...
  async ${(0, lodash_1.camelCase)(p.columnName)}Obj(
    ...
    const service = await ctx.reqCtx.requestContext.getAsync<${(0, helper_1.pascalCase)(p.referencedTableName)}Service>('${(0, lodash_1.camelCase)(p.referencedTableName)}Service');
...
import { Resolver, Query, Arg, Int, Mutation, ID, Ctx ${filedResolver ? ',FieldResolver, Root' : ''} } from 'type-graphql';
import {
  ${className},
  ${className}SaveIn,
  ${className}List,
